...
1. **배달 도메인 특화 감성 분석 AI 모델 파인튜닝 (Fine-Tuning)**
   - 배달 어플 리뷰를 크롤링하여 이진 분류(1~2점: 부정, 4~5점: 긍정) 라벨링 진행.
   - Hugging Face의 **RoBERTa** 모델을 기반으로 학습률(0.00002), 조기 종료(2), 반복 횟수(Epochs 3) 최적화를 거쳐 **약 95%의 높은 감성 분류 정확도(Accuracy)** 달성.
   - 해당 커스텀 모델(`ju03/Chatbot_Emotion-classification`)을 FastAPI 서버 메모리에 로드하여 실시간 추론 지원.
