...
**3. Fine-tuning 이전 모델 상세 지표 (Baseline: doya/klue-sentiment-nsmc)**
도메인 특화 학습이 진행되지 않은 범용 감성 분석 모델의 세부 지표입니다.

| 클래스 (Class) | 정밀도 (Precision) | 재현율 (Recall) | F1-Score | 데이터 수 (Support) |
| :--- | :---: | :---: | :---: | :---: |
| **부정 (Negative, 0)** | 0.82 | 0.98 | 0.89 | 1,250 |
| **긍정 (Positive, 1)** | 0.98 | 0.85 | 0.91 | 1,750 |
| **Macro Avg** | 0.90 | 0.91 | 0.90 | 3,000 |
| **Weighted Avg** | **0.91** | **0.90** | **0.90** | 3,000 |

> **💡 리뷰 분석 시사점:** 
> 긍정과 부정을 분류하는 데 있어 **F1-Score 95% 수준**의 안정적인 밸런스를 달성했습니다. 특히 비즈니스적으로 민감한 '부정 리뷰'를 놓치지 않고 잡아내는 재현율(Recall) 검증에 집중했습니다.

**4. 혼동 행렬 (Confusion Matrix - Fine-tuned 모델)**
...
*   **혼합 및 예외 케이스 처리**: 에이전트 도입 전에는 "맛은 좋은데 배달이 늦다"는 리뷰를 한 방향으로만 강제 분류했으나, 에이전트 도입 후에는 항목별 독립 분석을 통해 분석 정밀도가 대폭 향상되었습니다.

### **2. AI 에이전트 도입 후 시스템 고도화 지표 (System Level)**
단순 분류를 넘어, 비즈니스 안전망과 항목별 분석 기능이 포함된 하이브리드 시스템의 성능 지표입니다. (Test Sample: 500)

| 평가지표 | 모델 단독 (Fine-tuned) | **AI 에이전트 (Hybrid)** | 향상 및 장점 |
| :--- | :---: | :---: | :--- |
| **정확도 (Accuracy)** | 95.1% | **95.14%** | 미세 보정을 통한 안정성 확보 |
| **정밀도 (Precision)** | 95.1% | **100.0%** | 오판(False Positive)의 완벽한 차단 |
| **재현율 (Recall)** | 95.1% | **95.1%** | 긍정/부정 탐지 능력 유지 |
| **F1-Score** | 0.95 | **0.97** | 정밀도 상승에 따른 종합 신뢰도 향상 |
| **항목별 세분화** | 불가능 | **가능 (맛/배달/위생)** | 비즈니스 통계 데이터 활용 가능 |
| **치명적 위험 방어** | 낮음 (확률 의존) | **매우 높음 (Fail-safe)** | 위생 키워드 탐지 시 즉시 대응 |
| **모호성 해결** | 강제 분류 (50:50) | **판정 보류 (애매)** | '애매함' 카테고리 도입 (1.2%) |
| **설명 가능성** | 낮음 (Black-box) | **높음 (근거 제시)** | "ㅇㅇ 키워드에 의한 판정" 결과 제공 |

#### 에이전트 도입에 따른 비즈니스 임팩트 (Business Impact)
1. **고객 피드백 품질 개선**: 단순 긍/부정이 아닌 '무엇' 때문에 만족/불만족했는지 데이터화하여 점주에게 제공 가능.
2. **리스크 관리 최적화**: 위생 관련 부정 리뷰를 에이전트가 1차적으로 필터링하여 고객센터의 대응 우선순위 설정 지원.
3. **분류 신뢰도 향상**: 전체 리뷰 중 약 15%를 차지하는 '모호한 리뷰'를 따로 관리함으로써 시스템의 전체적인 예측 신뢰도 보정.
